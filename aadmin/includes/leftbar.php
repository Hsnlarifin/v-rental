<nav class="ts-sidebar">
	<ul class="ts-sidebar-menu">
		<li class="ts-label">Main</li>
		<li><a href="dashboard.php"><i class="fa fa-dashboard"></i> Dashboard</a></li>

<li><a href="#"><i class="fa fa-user"></i> Customer</a>
	<ul>
		<li><a href="reg-users.php">Reg Users</a></li>
		<li><a href="manage-cust.php">Manage Customer</a></li>
		<li><a href="payment-status.php">Update Payment Status</a></li>
	</ul>
</li>

<li><a href="#"><i class="fa fa-users"></i> Staff</a>
	<ul>
		<li><a href="new-staff.php">Add New Staff</a></li>
		<li><a href="manage-staff.php">Manage Staff</a></li>
		<li><a href="staff-branch.php">Total Staff by Branch</a></li>
	</ul>
</li>

<li><a href="#"><i class="fa fa-sitemap"></i> Bookings</a>
	<ul>
		<li><a href="manage-bookings.php">Manage Bookings</a></li>
		<li><a href="confirmed-bookings.php">View Confirmed</a></li>
	</ul>
</li>

<li><a href="#"><i class="fa fa-files-o"></i> Brands</a>
	<ul>
		<li><a href="create-brand.php">Create Brand</a></li>
		<li><a href="manage-brands.php">Manage Brands</a></li>
	</ul>
</li>

<li><a href="#"><i class="fa fa-car"></i> Vehicles</a>
	<ul>
		<li><a href="manage-vehicles.php">Manage Vehicles</a></li>	
		<li><a href="v-availability.php">Availability</a></li>
	</ul>
</li>



		<li><a href="dashboard.php"><i class="fa fa-bar-chart"></i> Report</a></li>
		<li><a href='#'><i class="fa fa-cloud"></i> Backup & Store</a></li>
	</ul>
</nav>
		

<!-- <li><a href="testimonials.php"><i class="fa fa-table"></i> Manage Testimonials</a></li>
<li><a href="manage-conactusquery.php"><i class="fa fa-desktop"></i> Manage Conatctus Query</a></li>
<li><a href="manage-pages.php"><i class="fa fa-files-o"></i> Manage Pages</a></li>
<li><a href="update-contactinfo.php"><i class="fa fa-files-o"></i> Update Contact Info</a></li>
<li><a href="manage-subscribers.php"><i class="fa fa-table"></i> Manage Subscribers</a></li> -->

